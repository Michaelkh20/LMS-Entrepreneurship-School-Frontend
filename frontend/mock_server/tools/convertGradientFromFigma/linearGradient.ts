...
export function getCorners(component: any, elemRotation: number) {
  ...
  const topLeft = [
    component.relativeTransform[0][2],
    component.relativeTransform[1][2],
  ];
  const topRight = rotate(
    topLeft[0],
    topLeft[1],
    topLeft[0] + component.size.x,
    topLeft[1],
    -elemRotation
  );
...
export function calculateAngle(x1: number, y1: number, x2: number, y2: number) {
  let angle =
    Math.round((180 / Math.PI) * Math.atan((y2 - y1) / (x2 - x1)) * 100) / 100;
...
export function createLinearGradient(fill: any, component: any) {
  ...
  let gradientAngle = calculateAngle(
  ...
  const lineChangeCoords = [
    (fill.gradientHandlePositions[1].x - fill.gradientHandlePositions[0].x) *
      component.size.x,
    (1 -
      fill.gradientHandlePositions[1].y -
      (1 - fill.gradientHandlePositions[0].y)) *
      component.size.y,
  ];
  ...
  const scaleCoords = [
    lineChangeCoords[0] * scaleFactor,
    lineChangeCoords[1] * scaleFactor,
  ];

  const elemRotate =
    -Math.acos(component.relativeTransform[0][0]) * (180 / Math.PI);
  const corners = getCorners(component, elemRotate);

  const shapeCenter = calculateIntersection(
    corners.topLeft,
    corners.bottomRight,
    corners.topRight,
    corners.bottomLeft
  ...
  const bottomLine = [
    rotate(
      centers.bottom[0],
      centers.bottom[1],
      centers.bottom[0] - desiredLength / 2,
      centers.bottom[1],
      elemRotate
    ),
    rotate(
  ...
  let absoluteStartingPoint = getGradientPoints(
    corners.topLeft,
    [
      fill.gradientHandlePositions[0].x * component.size.x,
      fill.gradientHandlePositions[0].y * component.size.y,
    ],
    [
      corners.topLeft[0] + component.size.x / 2,
      corners.topLeft[1] + component.size.y / 2,
    ],
    elemRotate
  );
  ...
  fill.gradientStops.map((color: any) => {
    ...
    let formattedColor = RGBAObject2String(
      colorObj,
      color.color.a * fillOpacity
    );
    ...
    let colorX =
      color.position * lineChangeCoords[0] + absoluteStartingPoint[0];
    let colorY =
      absoluteStartingPoint[1] - color.position * lineChangeCoords[1];
    let colorDistance = Math.sqrt(
      (colorY - gradientStartPoint.y) ** 2 +
        (colorX - gradientStartPoint.x) ** 2
    );
    let actualPercentage = colorDistance / gradientLineDistance;

    colorStr += `${formattedColor} ${
