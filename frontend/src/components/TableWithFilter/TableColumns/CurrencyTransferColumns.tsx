  ...
  [
    ...
    { title: 'Сумма', dataIndex: 'sum', key: 'sum' },
    {
      ...
      render: (_: any, record: CurrencyTransferColumnsDataType) => {
        return (
          <>
            {record.status === CLAIM_STATUS.Waiting && (
...
const ConfirmButtons = ({
  ...
  record: CurrencyTransferColumnsDataType;
}) => {
  return (
    <Space>
      <Popconfirm
        title={'Одобрить заявку?'}
        onConfirm={() => console.log('Approve', record.key, record.sum)}
      >
        <Button>Yes</Button>
      </Popconfirm>
      <Popconfirm
        title={'Отклонить заявку?'}
        onConfirm={() => console.log('Reject', record.key, record.sum)}
      >
        <Button>No</Button>
      </Popconfirm>
    </Space>
