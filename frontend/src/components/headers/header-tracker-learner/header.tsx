"use client"

import PersonIcon from '../../Icons/person-icon';
import MarketIcon from '../../Icons/market-icon';
import styles from './header.module.css';
import BackButton from '../../Buttons/BackButton/back-button';
import MarkIcon from '../../Icons/mark-icon';
import { Dropdown, Space } from 'antd';
import { items } from '../DropdownItems';

interface HeaderTrackerBigProps {
    balance: number;
    name: string;
    isHomePage: boolean;
    isTrackerPage: boolean;
  }
  
  const HeaderTrackerBig: React.FC<HeaderTrackerBigProps> = ({ balance, name, isHomePage, isTrackerPage }) => {
    return (
      <nav className={styles.nav}>
        <div className={styles.left}>
          {!isHomePage && <BackButton />}
        </div>
        <div className={styles.right}>
          <span className={styles.icon}>
            {!isTrackerPage && <MarkIcon />}
          </span>
          <span className={styles.iconMarket}>
            <MarketIcon />
          </span>
          <span className={styles.balance}>{balance} $</span>
          <Dropdown menu={{ items }}>
            <a onClick={(e) => e.preventDefault()}>
              <Space>
                <div className={styles.menu}>
                  <span className={styles.icon}>
                    <PersonIcon />
                  </span>
                  <span className={styles.name}> {name} </span>
                </div>
              </Space>
            </a>
          </Dropdown>
        </div>
      </nav>
    );
  };
  
  export default HeaderTrackerBig;