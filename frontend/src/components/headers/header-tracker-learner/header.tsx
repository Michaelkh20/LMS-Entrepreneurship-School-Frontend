...
interface HeaderBigProps {
  ...
  const NavBar: React.FC<HeaderBigProps> = ({ balance, name, isTrackerPage }) => {
    ...
    const isHomePage = isTrackerPage ? path === TRACKER_HOME_PAGE : path === LEARNER_HOME_PAGE;
    return (
      <nav className={styles.nav}>
        <div className={styles.left}>
          {!isHomePage && <Link href = {isTrackerPage?TRACKER_HOME_PAGE:LEARNER_HOME_PAGE}><BackButton/></Link>}
        </div>
        <div className={styles.right}>
          <span className={styles.icon}>
            {!isTrackerPage && <MarkIcon />}
          </span>
